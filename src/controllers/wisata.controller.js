...
async function reorderItineraryDay(req, res) {
  try {
    const data = await itineraryPlannerService.applyItineraryDayReorder(
      req.body,
    );
    ...
    res.status(isInputError ? 400 : 500).json({
      message: "Gagal mengurutkan ulang itinerary",
      error: error.message,
    });
  }
}

async function reorderItineraryAllDays(req, res) {
  try {
    const data = await itineraryPlannerService.applyItineraryAllDaysReorder(
      req.body,
    );
    ...
    res.status(isInputError ? 400 : 500).json({
      message: "Gagal mengurutkan ulang seluruh itinerary",
      error: error.message,
    });
  }
}
